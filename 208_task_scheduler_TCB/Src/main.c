...
const uint32_t init_psp_of_tasks[MAX_TASKS] = {T1_STACK_START, T2_STACK_START, T3_STACK_START, T4_STACK_START};
//fn ptrs void type
const uint32_t addr_of_task_handlers[MAX_TASKS] = { (uint32_t)task1_handler, (uint32_t)task2_handler, (uint32_t)task3_handler, (uint32_t)task4_handler };

//Task Control Block
typedef struct {
	uint32_t curr_psp_value;
